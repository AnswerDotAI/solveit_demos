...
Every item contains concrete instructions, possibly including copy/pasteable code, which shows how to do one specific task. Call it the FAT file -- Frequently Asked Tasks.
...
To install private GitHub repos, you can use Python to call the pip installer and pass in a repo address which contains your GitHub token.

For example, say you want to install the repo https://github.com/answerdotai/fastmigrate, where the GH user is `answerdotai` and the repo name is `fastmigrate`. 
...
Sharing a PDF is just a special case of sharing text, where you first need to process the PDF file into markdown which it is easier for the AI to read.
...
To define a tool for the AI, you simply expose a python function. So that the AI can understand what the function does, you must define it including parameter types and comments, using docments-style documentation.
...
SolveIt can render LaTeX expressions, so you can use the SolveIt AI to teach you about mathematical topics. 
...
Solveit can also render mathematical expressions generated by symbolic math libraries like `sympy`.
